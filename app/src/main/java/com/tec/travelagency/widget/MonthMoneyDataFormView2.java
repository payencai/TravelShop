 ...
 * 邮箱：771548229@qq..com
 */
public class MonthMoneyDataFormView2 extends View {
    ...
    private onIClickItem mOnIClickItem;

    public void setOnIClickItem(onIClickItem onIClickItem) {
        this.mOnIClickItem = onIClickItem;
    }
    ...
    public MonthMoneyDataFormView2(Context context) {
        this(context, null);
    }

    public MonthMoneyDataFormView2(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public MonthMoneyDataFormView2(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context, AttributeSet attrs) {
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.MonthMoneyDataFormView2);
        monthDataTextSize = ta.getDimensionPixelSize(R.styleable.MonthMoneyDataFormView2_monthMoneyDataTextSize2, R.dimen.text_size_14sp);
        isLastMonth = ta.getBoolean(R.styleable.MonthMoneyDataFormView2_isMoneyLastMouth2, false);
...
//                ToaskUtil.showToast(getContext(), "点击了第" + i + "行,第" + j + "列，号数：" + s);
                if (index != -1) {
                    mOnIClickItem.onClickItem(point);
                }
            }
        }
...
//        canvas.drawText(orderNum, left + (mRowSize / 2), (i * mLineSize) + 4 * (averageNum / 2) + weekRounds.height() / 2, mTextPaint);
        ...
        String flag = "i=" + i + "j=" + j;
        Point point = new Point(text, Double.parseDouble(price), Double.parseDouble(orderNum), index, isLastMonth ? (curMouthNum + 1) : curMouthNum);
        locationMap.put(flag, point);
...
//                System.out.println(calendar.get(Calendar.YEAR) + "的" + (calendar.get(Calendar.MONTH) + 1) + "月1号是 星期" + week[today - 1]);
    ...
    public static class Point {
        ...
        private int drawMonNumber;//当前画的月数
...
//        }
        ...
        public Point(String text, double roomTotal, double singular, int index, int drawMonNumber) {
            this.text = text;
            this.roomTotal = roomTotal;
            this.singular = singular;
            this.index = index;
            this.drawMonNumber = drawMonNumber;
        }
        ...
        public int getDrawMonNumber() {
            return drawMonNumber;
        }

        public void setDrawMonNumber(int drawMonNumber) {
            this.drawMonNumber = drawMonNumber;
        }
        ...
        public String toString() {
            return "Point{" +
                    ...
                    ", singular=" + singular +
                    ", index=" + index +
                    ", drawMonNumber=" + drawMonNumber +
                    '}';
        }
    }

    public interface onIClickItem {
        public void onClickItem(Point point);
    }
