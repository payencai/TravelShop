...
public class StarLinearLayout extends LinearLayout implements View.OnClickListener {
    ...
    public StarLinearLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        if (attrs != null) {
            TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.StarLinearLayout);
            mMargin = (int) a.getDimension(R.styleable.StarLinearLayout_margin, 10);
            isEdit = a.getBoolean(R.styleable.StarLinearLayout_isEdit, false);
            mScore = a.getFloat(R.styleable.StarLinearLayout_score, 0);
            a.recycle();
        }
        init();
        setScore(mScore);
