...
/**
 * 作者：凌涛 on 2018/8/23 14:47
 * 邮箱：771548229@qq..com
 */
public class ScenicBean extends RVBaseCell implements Serializable{
    ...
    public RVBaseViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_door_scenic_rv_layout, parent, false);
    ...
    public void onBindViewHolder(RVBaseViewHolder holder, int position, final Context context, RVBaseAdapter adapter) {
        ...
        Glide.with(context).load(image1Url).into(headImg);

        holder.setText(R.id.name, name);
        holder.setText(R.id.address, address);
//        holder.setText(R.id.scenicDes, introduce);

        SpannableString platformPrice = new SpannableString("￥" + 450 + "起");
        int color = context.getResources().getColor(R.color.price_text_color);
        ForegroundColorSpan span = new ForegroundColorSpan(color);
        platformPrice.setSpan(span, 0, platformPrice.length() - 1, Spannable.SPAN_EXCLUSIVE_INCLUSIVE);
        holder.setText(R.id.price, platformPrice);

        holder.getView(R.id.item).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(context, DoorTicketActivity.class);
                intent.putExtra("scenicBean", ScenicBean.this);
