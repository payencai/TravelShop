 ...
 */
public class CarPlatFragment extends BaseFragment {
    ...
    CarPlatAdapter mTicketPlatAdapter;
    ...
    public static CarPlatFragment newInstance(String from) {
        Bundle args = new Bundle();
        args.putString("from", from);
        CarPlatFragment fragment = new CarPlatFragment();
    ...
    protected int getContentId() {
        return R.layout.fragment_car_plat;
    }
    ...
    private void initRecycle() {
        ...
        mTicketPlatAdapter = new CarPlatAdapter(R.layout.item_car);
    ...
    private void requestData(String name, String isAdapot, String province, String city, String district) {
        ...
        HttpProxy.obtain().get(PlatformContans.TravelAgency.getPlatCarList, App.getInstance().getUserEntity().getToken(), parame, new ICallBack() {
            @Override
            public void OnLtSuccess(String result) {
                ...
                try {
                    ...
                    if (resultCode == 0) {
                        ...
                        Gson gson = new Gson();
                        List<PlatCarBean> tempList = new ArrayList<>();
                        for (int i = 0; i < length; i++) {
                            JSONObject item = beanList.getJSONObject(i);
                            PlatCarBean bean = gson.fromJson(item.toString(), PlatCarBean.class);
                            tempList.add(bean);
                        }
                        Log.e("platcar", result);
                        if (isLoadMore) {
