...
import com.tec.travelagency.home.activity.route.Route;
import com.tec.travelagency.home.activity.route.SalePathBean;
import com.tec.travelagency.home.activity.route.TravelAgencyRoute;
...
public class TicketsDetailActivity extends BaseActivity implements RvItemActionListener {
    ...
    SaleTicketsBean mSaleTicketsBean;

    private List<SaleTicketsDetailBean> mList = new ArrayList<>();
    ...
    private SaleTicketSubAdapter mRoomDetailAdapter;
    ...
    protected void initView() {
        ...
        comment.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                CommentActivity.startCommentActivity(TicketsDetailActivity.this,4,mSaleTicketsBean.getId());
            }
        });
        initRv();
    ...
    private void getSaleTicketsDetail(String id) {
        ...
        map.put("id", id);
        HttpProxy.obtain().get(PlatformContans.Ticket.getSaleTicketsDetail, App.getInstance().getUserEntity().getToken(), map, new ICallBack() {
            @Override
            public void OnLtSuccess(String result) {
                ...
                try {
                    ...
                    if (resultCode == 0) {
                        ...
                        mSaleTicketsBean = new Gson().fromJson(data.toString(), SaleTicketsBean.class);
                        setData();
                    } else {
    ...
    private void requestHotelRoomData(String id) {
        ...
        HttpProxy.obtain().get(PlatformContans.Ticket.getSaleTicketsDetailList, App.getInstance().getUserEntity().getToken(), parame, new ICallBack() {

            @Override
            public void OnLtSuccess(String result) {
                LogUtil.Log("sale", result);
                try {
                    JSONObject object = new JSONObject(result);
                    int resultCode = object.getInt("resultCode");
                    ...
                    if (resultCode == 0) {
                        ...
                        for (int i = 0; i < length; i++) {
                            ...
                            SaleTicketsDetailBean bean = gson.fromJson(item.toString(), SaleTicketsDetailBean.class);
                            mList.add(bean);
                        }
                        mRoomDetailAdapter.notifyDataSetChanged();

                    } else {
                        ToaskUtil.showToast(TicketsDetailActivity.this, message);
                    }
                } catch (JSONException e) {
                    e.printStackTrace();
                }
    ...
    private void initRv() {
        ...
        mRoomDetailAdapter = new SaleTicketSubAdapter(this, mList);
     ...
     * @param roomDetail
     */
    private void getRoomTypeDetail(final SaleTicketsDetailBean roomDetail) {
        Map<String, Object> map = new HashMap<>();
        map.put("id", roomDetail.getId());
        HttpProxy.obtain().get(PlatformContans.Ticket.getSaleTicketsSubDetail, App.getInstance().getUserEntity().getToken(), map, new ICallBack() {
            @Override
            public void OnLtSuccess(String result) {
                ...
                try {
                    ...
                    if (resultCode == 0) {
                        JSONObject data = object.getJSONObject("data");
                        SaleTicketsDetailBean bean = new Gson().fromJson(data.toString(), SaleTicketsDetailBean.class);
    ...
    private void showRoomDetailDialog(SaleTicketsDetailBean roomDetail) {
        View view = LayoutInflater.from(this).inflate(R.layout.dialog_ticket, null);
        ButtomDialogView dialogView = new ButtomDialogView(this, view, Gravity.BOTTOM);
        dialogView.show();
        setDialogData(view, roomDetail, dialogView);
    }
    TextView adapot;
    private void setDialogData(View view, final SaleTicketsDetailBean meetingBean, final Dialog dialog) {
        ...
        TextView selling_price = view.findViewById(R.id.selling_price);
         adapot = view.findViewById(R.id.sold_out);
        ...
        regulation.setText(meetingBean.getRegulation());
        TextView updatePrice = view.findViewById(R.id.updatePrice);
        selling_price.setText("售价: ￥" + meetingBean.getPublishPrice());
        final ImageView cancel = view.findViewById(R.id.cancel);
        // platform_price.setText("平台价: ￥" + meetingBean.getOneSignPrice());
        periodOfValidity.setText(meetingBean.getPeriodOfValidity());
        warmPrompt.setText(meetingBean.getWarmPrompt());
        if (meetingBean.getIsUsed().equals("2")) {
            adapot.setText("上架");
        } else {
            adapot.setText("下架");
        }
        if (meetingBean.getIsCanCancel().equals("1")) {
            label1.setText("可退");
        } else {
            label1.setText("不可退");
        }
        ...
        adapot.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (meetingBean.getIsUsed().equals("2"))
                    adopt(meetingBean,dialog);
                else if (meetingBean.getIsUsed().equals("1")) {
                    cancel(meetingBean,dialog);
                }
            }
        });
        ...
        Glide.with(this).load(meetingBean.getImage1Url()).into(head);

    }

    private void showDialog(final SaleTicketsDetailBean hotelRoomTypeDetail, final int type) {
        ...
        confrim.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (type == 0) {
                    ...
                    //adopt(hotelRoomTypeDetail,dialogView);
                } else if (type == 1) {
...
//    }
        ...
        HttpProxy.obtain().post(PlatformContans.Meeting.updateMeetingPrice, App.getInstance().getUserEntity().getToken(), parame, new ICallBack() {
            ...
            public void OnLtSuccess(String result) {
                ...
                try {
                    ...
                    String message = object.getString("message");
                    if (resultCode == 0) {
                        ToaskUtil.showToast(TicketsDetailActivity.this, "修改成功");
                        getSaleTicketsDetail(id);
                        requestHotelRoomData(id);
                    } else {
                        ToaskUtil.showToast(TicketsDetailActivity.this, message);
     ...
     * @param roomDetail
     */
    private void adopt(SaleTicketsDetailBean roomDetail, final Dialog dialog) {
        Map<String, Object> parame = new HashMap<>();
        parame.put("id", roomDetail.getId());
//        parame.put("price", price);
        HttpProxy.obtain().post(PlatformContans.Ticket.upSaleTickets, App.getInstance().getUserEntity().getToken(), parame, new ICallBack() {
            @Override
            public void OnLtSuccess(String result) {
                ...
                try {
                    ...
                    String message = object.getString("message");
                    if (resultCode == 0) {
                        ToaskUtil.showToast(TicketsDetailActivity.this, "上架成功");
                        adapot.setText("下架");
                        dialog.dismiss();
                        getSaleTicketsDetail(id);
                        requestHotelRoomData(id);
     ...
     * @param roomDetail
     */
    private void cancel(SaleTicketsDetailBean roomDetail, final Dialog dialog) {
        Map<String, Object> parame = new HashMap<>();
        parame.put("id", roomDetail.getId());
        HttpProxy.obtain().post(PlatformContans.Ticket.downSaleTickets, App.getInstance().getUserEntity().getToken(), parame, new ICallBack() {
            @Override
            public void OnLtSuccess(String result) {
                ...
                try {
                    ...
                    String message = object.getString("message");
                    if (resultCode == 0) {
                        ToaskUtil.showToast(TicketsDetailActivity.this, "下架成功");
                        adapot.setText("上架");
                        dialog.dismiss();
                        getSaleTicketsDetail(id);
                        requestHotelRoomData(id);
    ...
    public void OnItemClick(int position) {
        SaleTicketsDetailBean roomDetail = mList.get(position);
