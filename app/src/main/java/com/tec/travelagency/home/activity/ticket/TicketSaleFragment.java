...
import com.tec.travelagency.home.activity.route.SalePathAdapter;
import com.tec.travelagency.home.activity.route.SalePathBean;
import com.tec.travelagency.home.activity.route.SalePathFragment;
 ...
 */
public class TicketSaleFragment extends BaseFragment {
    ...
    SaleTicketsAdapter mTicketPlatAdapter;
    ...
    public static TicketSaleFragment newInstance(String from) {
        Bundle args = new Bundle();
        args.putString("from", from);
        TicketSaleFragment fragment = new TicketSaleFragment();
    ...
    protected int getContentId() {
        return R.layout.fragment_sale_path;
    }
    ...
    private void initRecycle() {
        ...
        mTicketPlatAdapter = new SaleTicketsAdapter(R.layout.item_ticket);
        mTicketPlatAdapter.setOnItemClickListener(new BaseQuickAdapter.OnItemClickListener() {
            @Override
            public void onItemClick(BaseQuickAdapter baseQuickAdapter, View view, int i) {
                Intent intent = new Intent(getContext(), TicketsDetailActivity.class);
                intent.putExtra("flag", "1");
                intent.putExtra("id", mTicketPlatAdapter.getData().get(i).getId());
...
//        mTicketPlatAdapter.setNewData(getData());
//        rv_ticket1.setAdapter(mTicketPlatAdapter);
    ...
    private void requestData(String name, String starLevel, String province, String city, String district) {
        ...
        if (!TextUtils.isEmpty(starLevel)) {
            parame.put("starLevel", starLevel);
        }
        Log.e("params",parame.toString());
        HttpProxy.obtain().get(PlatformContans.Ticket.getSaleTicketsList, App.getInstance().getUserEntity().getToken(), parame, new ICallBack() {
            @Override
            public void OnLtSuccess(String result) {
                if (mSwipeRefreshLayout.isRefreshing()) {
                    mSwipeRefreshLayout.setRefreshing(false);
                }
                Log.e("getSaleTicketsList", result);
                try {
                    JSONObject object = new JSONObject(result);
                    int resultCode = object.getInt("resultCode");
                    ...
                    if (resultCode == 0) {
                        ...
                        Gson gson = new Gson();
                        List<SaleTicketsBean> tempList = new ArrayList<>();
                        for (int i = 0; i < length; i++) {
                            JSONObject item = beanList.getJSONObject(i);
                            SaleTicketsBean bean = gson.fromJson(item.toString(), SaleTicketsBean.class);
                            tempList.add(bean);
                        }
                        if (isLoadMore) {
