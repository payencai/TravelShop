...
import com.tec.travelagency.orderManager.entity.LxsPathOrder;
 ...
 */
public class UserPathOrderFragment extends AbsBaseFragment<UserPathOrder> {

    private int page = 0;
    private boolean isLoadMore = false;
    private int mCategoryId = 5;//订单类型

    public static UserPathOrderFragment getInstance(int categoryId) {
        UserPathOrderFragment baseFragment = new UserPathOrderFragment();
    ...
    @Override
    protected List<Cell> getCells(List<UserPathOrder> list) {
    ...
    private void requestData() {
        ...
        HttpProxy.obtain().get(PlatformContans.UserOrder.getUserPathOrderList, App.getInstance().getUserEntity().getToken(), parame, new ICallBack() {
            @Override
            public void OnLtSuccess(String result) {
                ...
                try {
                    ...
                    if (resultCode == 0) {
                        ...
                        Gson gson = new Gson();
                        List<UserPathOrder> list = new ArrayList<>();
                        int pageNum = data.getInt("pageNum");
                        for (int i = 0; i < length; i++) {
                            JSONObject item = beanList.getJSONObject(i);
                            UserPathOrder bean = gson.fromJson(item.toString(), UserPathOrder.class);
                            list.add(bean);
                        }
