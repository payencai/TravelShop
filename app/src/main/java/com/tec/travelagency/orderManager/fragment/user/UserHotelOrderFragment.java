...
import com.tec.travelagency.orderManager.entity.UserHotelOrder;
 ...
 * 邮箱：771548229@qq..com
 */
public class UserHotelOrderFragment extends AbsBaseFragment<UserHotelOrder> {

    private int page = 0;
    private boolean isLoadMore = false;


    public static UserHotelOrderFragment getInstance(int categoryId) {
        UserHotelOrderFragment baseFragment = new UserHotelOrderFragment();
        return baseFragment;
    }
    ...
    @Override
    protected List<Cell> getCells(List<UserHotelOrder> list) {
        return null;
    }
    ...
    private void requestData() {
        ...
        HttpProxy.obtain().get(PlatformContans.UserOrder.getUserHotelOrderList, App.getInstance().getUserEntity().getToken(), parame, new ICallBack() {
            @Override
            public void OnLtSuccess(String result) {
                ...
                try {
                    ...
                    if (resultCode == 0) {
                        ...
                        Gson gson = new Gson();
                        List<UserHotelOrder> list = new ArrayList<>();

                        int pageNum = data.getInt("pageNum");
                        for (int i = 0; i < length; i++) {
                            JSONObject item = beanList.getJSONObject(i);
                            UserHotelOrder bean = gson.fromJson(item.toString(), UserHotelOrder.class);
                            list.add(bean);
                        }
