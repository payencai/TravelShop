...
import com.tec.travelagency.orderManager.entity.LxsTicketsOrder;
import com.tec.travelagency.orderManager.entity.UserPathOrder;
import com.tec.travelagency.orderManager.entity.UserTicketsOrder;
 ...
 */
public class UserTicketsOrderFragment extends AbsBaseFragment<UserTicketsOrder> {

    private int page = 0;
    private boolean isLoadMore = false;
    private int mCategoryId = 5;//订单类型

    public static UserTicketsOrderFragment getInstance(int categoryId) {
        UserTicketsOrderFragment baseFragment = new UserTicketsOrderFragment();
    ...
    @Override
    protected List<Cell> getCells(List<UserTicketsOrder> list) {
    ...
    private void requestData() {
        ...
        HttpProxy.obtain().get(PlatformContans.UserOrder.getUserTicketsOrderList, App.getInstance().getUserEntity().getToken(), parame, new ICallBack() {
            @Override
            public void OnLtSuccess(String result) {
                ...
                try {
                    ...
                    if (resultCode == 0) {
                        ...
                        Gson gson = new Gson();
                        List<UserTicketsOrder> list = new ArrayList<>();
                        int pageNum = data.getInt("pageNum");
                        for (int i = 0; i < length; i++) {
                            JSONObject item = beanList.getJSONObject(i);
                            UserTicketsOrder bean = gson.fromJson(item.toString(), UserTicketsOrder.class);
                            list.add(bean);
                        }
