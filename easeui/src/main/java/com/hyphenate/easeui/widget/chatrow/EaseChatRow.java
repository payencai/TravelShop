    ...
    private void setUpBaseView() {
        ...
        if (userAvatarView != null) {
            ...
            if (message.direct() == Direct.SEND) {//
                String meUserHeadUrl = EaseUserUtils.getMeUserHeadUrl();
                if (TextUtils.isEmpty(meUserHeadUrl)) {
                    EaseUserUtils.setUserAvatar(context, EMClient.getInstance().getCurrentUser(), userAvatarView);
                } else {
                    EaseUserUtils.setUserAvatar2(context, meUserHeadUrl, userAvatarView);
                }
                EaseUserUtils.setUserNick(EaseUserUtils.userName,usernickView);
            } else {
                String mNikeName = null;
                String mHeadimg = null;
                String mHeadimgKey = null;
                try {
                    mNikeName = message.getStringAttribute("nikename");
                } catch (HyphenateException e) {
                    e.printStackTrace();
                }
                try {
                    mHeadimg = message.getStringAttribute("headimg");
                } catch (HyphenateException e) {
                    e.printStackTrace();
                }
                try {
                    mHeadimgKey = message.getStringAttribute("headimgKey");
                } catch (HyphenateException e) {
                    e.printStackTrace();
                }
//                EaseUserUtils.setUserAvatar(context, message.getFrom(), userAvatarView);
                EaseUserUtils.setUserAvatar2(context, mHeadimg, userAvatarView);
                if (TextUtils.isEmpty(mNikeName)) {
                    EaseUserUtils.setUserNick(message.getFrom(), usernickView);
                } else {
                    EaseUserUtils.setUserNick(mNikeName, usernickView);
                }
            }
        }
